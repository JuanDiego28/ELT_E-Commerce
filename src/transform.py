...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data_aux = pd.merge(orders, items, on="order_id", how="inner")
    data = pd.merge(data_aux, products, on="product_id", how="inner")
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_ammount_per_date = (
        filtered_dates.groupby(by="order_purchase_timestamp")
        .count()
        .reset_index()
        .loc[:, ["order_id", "order_purchase_timestamp"]]
    )

    # Creating a dataframe with the result, rename columns
    result_df = order_purchase_ammount_per_date.rename(
        columns={"order_purchase_timestamp": "date", "order_id": "order_count"}
    )
